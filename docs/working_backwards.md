...
accurate answers with supporting charts.
...
- [x] 11 trusted KPI functions
- [x] 5-page Streamlit app (Dashboard, Products, Customers, Inventory, Chat)
- [x] AI chat with Anthropic tool use
- [x] Test suite: 112 tests, 100% passing
- [x] Architecture and design documentation

### M1 — Demo-ready (target: 2 weeks)
- [ ] README with architecture diagram and setup instructions
- [ ] `.env` setup and API key working end-to-end
- [ ] Record 3-minute demo video (walk through the chat with real questions)
- [ ] Deploy to a shareable URL (Streamlit Cloud or Fly.io)
- [ ] Clean GitHub repo (public or invite-only)

### M2 — Pilot client (target: 6 weeks)
- [ ] Identify 1 real lumber/supply business to pilot
- [ ] Build CSV upload flow (no integration required — just upload exports)
- [ ] QuickBooks export parser (most SMBs use QuickBooks)
- [ ] Onboarding script: data audit → mapping → load → first dashboard
- [ ] Collect feedback: which questions matter most, where data breaks

### M3 — First paying client (target: 3 months)
- [ ] Fix top issues from pilot feedback
- [ ] Add 3–5 new KPI functions based on real questions asked
- [ ] Move storage to Postgres (pilot scale)
- [ ] Add user auth (single login, one client per deployment)
- [ ] Price the service and invoice the first client

### M4 — Second vertical (target: 6 months)
- [ ] Identify second vertical (field services or HVAC/contractor)
- [ ] Clone project, adapt schema and KPIs for new domain
- [ ] Build client #2 based on what worked in M3

---

## Task Breakdown (M1 in detail)

Assuming 1 senior engineer (you) working in focused sessions:

| Task | Est. | Notes |
|------|------|-------|
| Write README | 2h | Architecture overview, setup steps, demo flow |
| Deploy to Streamlit Cloud | 1h | Free tier, needs GitHub repo |
| Record demo video | 1h | Screen + voiceover, 3 min max |
| Polish chat suggestions | 30m | Better example questions for the demo |
| Create GitHub repo | 30m | Private or public — decide intentionally |
| Set up `.env` and verify end-to-end | 30m | Already done locally, verify in deployed env |

**Total M1 effort: ~6 hours of focused work.**

---

## Parallelization Plan (if headcount available)

This is how I would staff this if delivering as a team:
...
Engineer B (Full-stack / Mid-level)
  → Frontend polish, chart components, UI improvements
...
| Integration with POS takes 3x longer than expected | High | Medium | Don't build integrations until post-M3 |

---

## Definition of Success at Each Stage

**M1 success:** Someone with no context watches the demo and says "I'd pay for that."

**M2 success:** A real business owner uses the product without hand-holding and finds
a real insight they didn't have before.
